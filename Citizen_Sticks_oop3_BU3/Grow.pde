...
class Grow {
  ...
  void blockGrowthMaster(float[][] ar, float[][] ag, float[][] ab, color kr, color kg,color kb, ArrayList cr,ArrayList cg,ArrayList cb,int g) {

      // if (inputArray.length>0) {
             
        for (int i=0; i<ar.length; i++) {                                                    
                Vec3D origin =  new Vec3D(int(ar[i][0]/gridSize)*gridSize, int(ar[i][1]/gridSize)*gridSize, 0);  
                Block curB =  new Block(origin, this, cr, kr,g); 
                cr.add(curB);           
            }
       ...
       //////////////////
          ...
          for (int i=0; i<ag.length; i++) {                                                    
                ...
                Block curB =  new Block(origin, this, cg, kg,g); 
                cg.add(curB);           
            }
        ...
        ////////////////////////////
           for (int i=0; i<ab.length; i++) {                                                    
                ...
                Block curB =  new Block(origin, this, cb, kb,g); 
                cb.add(curB);           
            }
  ...
  void pushToScreen() {
    ...
    blockCollectionR, blockCollectionG, blockCollectionB,gridSize);
  }
...
class Block { 
 ...
 // Vec3D speed = new Vec3D(random(-2, 2), random(-2, 2), 0);
  ...
  //Constructor
  Block (Vec3D _loc, Grow _p, ArrayList _c, color _k, int _g) {  
    loc = _loc;
    p = _p;
    c = _c;
    k = _k;
  ...
  void lineBetween() {
    ...
    for (int i = 0; i < c.size(); i++ ) {
      ...
      if ( distance > 70 && distance < 90) {
        ...
        rectMode(CENTER);
        fill(k);
        rect(loc.x, loc.y, grids/2,grids/2,3);
      }
    }
  }
  ...
  void display() { 
        ...
        fill(k);
        stroke(k);
        rect(loc.x, loc.y, grids,grids,6); 
  } 
  void move() {
