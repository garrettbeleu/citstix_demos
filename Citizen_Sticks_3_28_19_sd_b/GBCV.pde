...
class GBCV {
  ...
  GBCV() {
    ...
    //matRed = new Mat(1280,720, CvType.CV_8UC1); //red hues 0-10
    //matRed2 = new Mat(1280,720, CvType.CV_8UC1); //red hues 150-179
    //matGreen = new Mat(1280,720, CvType.CV_8UC1);
    //matBlue = new Mat(1280,720, CvType.CV_8UC1);
    //matRGB = new Mat(1280,720, CvType.CV_8UC3);
    //
    circlesRed= new Mat();
  ...
  Imgproc.HoughCircles(input, output, Imgproc.CV_HOUGH_GRADIENT, 
    ...
    if (output.rows()>0) {
      ...
      //added 4/23
      float maxSticksValue = objui.maxSticks.getValue();
      if(tempData.length <= maxSticksValue) { //capture them all if less than max
      ...
      if (tempData.length > maxSticksValue ) {
        theData.data = new float[int(maxSticksValue)][3];
        for(int i=0; i<maxSticksValue; i++) {
